...
Fuse's [Castlevania: Aria of Sorrow Randomizer 2](https://aosrando.surge.sh/) natively lacks the ability to generate fully-human-readable spoiler log for rolled seeds. It does, however, produce browser console output for logging/debug purposes. The data output to the browser console when clicking the `Randomize` button on Fuse's randomizer can be saved to a `.txt` file and uploaded to this project's webpage, which will then perform a translation of the information in the logs and a output a significantly more human readable spoiler log `.txt` file for the given seed.
...
Unfortunately the logs for AoSR2 and the reference tables in AoSR1 both lack key information required to deliver the information in that second list (particularly which numbers in the logs refer to which enemies/bosses).
